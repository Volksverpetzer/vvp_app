 ...
 *
 * Excludes /wp-content/uploads/ and /wp-admin/ from being intercepted as
 * app links so the OS can handle them directly (e.g. open a PDF in the
 * browser). On API < 31, pathAdvancedPattern is silently ignored and the
 * element matches all paths — the app-code fallback in index.tsx then
 * calls Linking.openURL to hand the URL to the OS.
 */
